...
class CrystalSTEMSimulation:
    def __init__(self, phase,
        ...
        self.simulation  = self.generator.calculate_ed_data(phase=phase,
                                                            reciprocal_radius=1,
                                                            rotation=r,
        ...
        self.centers = np.random.randint(low=crystal_size+5,
        ...
        self.vectors, self.coordinates, self.real_space_vectors, self.intensities = self.get_nano_crystal_vectors(max_excitation_error=max_excitation_error)
    ...
    def get_nano_crystal_vectors(self,max_excitation_error=0.015):
        ...
        dataset = np.zeros((real_space_pixels,
                            real_space_pixels,
                            recip_space_pixels, 
                            recip_space_pixels))
        ...
        for rot, center, inplane in zip(self.random_rotations, 
                                        self.centers,
                                        self.random_inplane):
            ...
            vector_pos = np.argwhere(create_blob(center=center,
    ...
    def make_4d_nano(self,
        ...
        vectors_by_index = convert_to_markers(self.vectors,
                                              real_space=self.real_space_pixels)

        arr = np.zeros((real_space_pixels,
                        real_space_pixels, 
                        recip_space_pixels,
                        recip_space_pixels))
    ...
    def plot_real_space(self, 
        ...
        colors = ["blue", "green", "red", "yellow", "purple",
                  "orange", "violet", "indigo", "black", "grey"]
        verts = []
        if remove_below_n is not None:
            vectors = [v for v,c in zip(self.real_space_vectors, self.coordinates) if len(c)>remove_below_n]
        else:
        ...
        if remove_non_symmetric:
            ...
            for c,v in zip(self.coordinates, self.real_space_vectors):
                un, counts = np.unique(np.round(np.linalg.norm(c[:,:2], axis=1), 2), return_counts=True)
                if np.any(counts>2):
    ...
    def plot_example_dp(self,
        ...
        sim = self.generator.calculate_ed_data(self.phase, 
                                               rotation=rotation,
                                               reciprocal_radius=reciprocal_radius,
                                               max_excitation_error=.01,
        ...
        ax.imshow(img,extent=(-reciprocal_radius,reciprocal_radius,-reciprocal_radius,reciprocal_radius), **kwargs)
        return ax
...
def convert_to_markers(peaks,
    ...
    high_x_ind = np.searchsorted(sorted_peaks[:,0],range(1,real_space+1), side="left")
    for i, (lo_x,hi_x) in enumerate(zip(low_x_ind, high_x_ind)):
        ...
        high_y_ind = np.searchsorted(x_inds[:,1],range(1,real_space+1), side="left")
...
import numpy as np
from copy import deepcopy
def convert_flat_to_markers(peaks,
                            signal,
                            **kwargs):
    new_peaks = deepcopy(peaks)
...
def get_filtered_combinations(pks,
    ...
    if min_k is not None:
        in_k_range = np.array([np.mean(np.abs(np.subtract(np.mean(k), k))) < min_k for k in k_combos])
    else:
        in_k_range=True
...
def get_three_angles(pks,
    ...
    combos, combo_k = get_filtered_combinations(pks,3,
                                           ...
                                           intensity_threshold=intensity_threshold,min_angle=min_angle, min_k=min_k)
...
def get_vector_centers(vect, label_ind=-2, columns=[2,3]):
    ...
    centers = np.array([np.mean(vect[vect[:,label_ind]==l], axis=0) for l in sub_labels])
